...
def plot_pie_chart(user_responses):
    ...
    investment_capacity = total_income - total_costs - costs_var

    # Data for the pie chart
    sizes = [costs_var, transportation_costs, food_costs, outing_expenses, other_costs, total_costs, investment_capacity]
    ...
    if len(np.unique(sizes)) != 1 or np.unique(sizes)[0] != 0 :
        ...
        plt.pie(sizes, labels=None, autopct='%1.1f%%', startangle=90, textprops={'fontsize': 10})  # Adjust autopct font size
        plt.axis('equal')  # Ensures that pie is drawn as a circle.
        plt.title('Distribution of Costs and Investment capacity')
        plt.legend(labels, loc="best", fontsize="small")  # Add legend
        plt.show()
    else:
        return 
